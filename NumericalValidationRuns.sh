#!/bin/bash                                                                                                                                                                     
gcc -O3 -march=native -o num_val_Aug25.out TwoCurrSWEsolver.c # Compile main
...
#    ./num_val_Aug25.out 20000 1000 0.1 0.0001 $s $u_s 1.0 1.0 &
#  done
#done
...
for u_s in $U_s_list; do
  for Re in $Re_list; do
    #mkdir "outputs/1_1_${runind}_outputs/" 
    ./num_val_Aug25.out 28000 $Re 0.1 0.0001 200 $u_s 1.0 1.0 &
  done
done
...
for u_s in $U_s_list; do
  for h in $h_list; do
    #mkdir "outputs/1_1_${runind}_outputs/" 
    ./num_val_Aug25.out 28000 1000 0.1 $h 200 $u_s 1.0 1.0 &
  done
done
# done
...
for u_s in $U_s_list; do
  for N in $N_list; do
    #mkdir "outputs/1_1_${runind}_outputs/" 
    ./num_val_Aug25.out $N 1000 0.1 0.0001 200 $u_s 1.0 1.0 &
  done
done
# done
...
for u_s in $U_s_list; do
  for CFL in $CFL_list; do
    #mkdir "outputs/1_1_${runind}_outputs/" 
    ./num_val_Aug25.out 28000 1000 $CFL 0.0001 200 $u_s 1.0 1.0 &
  done
done
# done
